...
| Per-date synthesis | When multiple dates are pending, runs one LLM call per date to prevent multi-day collapse into a single daily file; in-session path combines into single sequential subagent to avoid LTM write conflicts |
| Synthesis error surfacing | `synthesis_cron.py` logs failures to `.synthesis-errors.log`; `load_memory.py` checks log at SessionStart, injects alert, and clears log (one-shot); failed runs clear eager timestamp so retries work |
