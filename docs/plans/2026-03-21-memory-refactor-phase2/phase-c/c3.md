...
Add a `MemoryOp` dataclass and update `SynthesisResult`:
...
class SynthesisResult:
    ...
    memory_ops: list[MemoryOp] = field(default_factory=list)  # NEW
    warnings: list[str] = field(default_factory=list)
```

When parsing JSON ops, convert each dict to a `MemoryOp` instance for type safety.
